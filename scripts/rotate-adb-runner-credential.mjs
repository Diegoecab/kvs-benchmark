...
const image = options.image || "ghcr.io/diegoecab/kvs-benchmark-runner@sha256:7bf7c3d1d3d5ae1b650ca38f8434ec545572bec6a7c07bdd3829b0f29bb392c9";
...
const installScript = `#!/usr/bin/env bash\nset -euo pipefail\nsudo -n podman run --rm --user 0:0 -v /opt/kvs-dashboard:/secure:Z -e KVS_ADMIN='${encryptedPassword}' -e KVS_DB='${safe(options["database-id"])}' -e KVS_REGION='${safe(options.region)}' -e KVS_ENDPOINT='${safe(endpoint)}' -e KVS_AUTH='${safe(authEndpoint)}' -e KVS_TABLE='${safe(options.table)}' --entrypoint node '${image}' --input-type=module --eval '${provisionJavascript}'\n`;
