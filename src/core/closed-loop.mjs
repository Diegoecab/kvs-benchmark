...
export async function runClosedLoop({ config, configSha256, provider, target, table, output, startAt }) {
  ...
  const errors = {};
  let sequence = 0, completed = 0, failed = 0, active = 0, peakInflight = 0, readUnits = 0, writeUnits = 0, retries = 0;
  const cpuStart = process.cpuUsage();
  const loopDelay = monitorEventLoopDelay({ resolution: 10 }); loopDelay.enable();
  const telemetryTimer = setInterval(() => {
    concurrency.push(active);
    telemetryOutput.write(`${JSON.stringify({ at: new Date().toISOString(), inFlight: active, rssBytes: process.memoryUsage().rss, heapUsedBytes: process.memoryUsage().heapUsed })}\n`);
  ...
  await Promise.all(Array.from({ length: config.load.fixedConcurrency }, (_, workerId) => worker(workerId + 1)));
  ...
  const summary = {
    ...
    errors, schedulerDrops: 0, retries, workload: { readPercent: config.workload.readPercent, writePercent: config.workload.writePercent, writeMode: config.workload.writeMode || null, executionMode: "fixed-concurrency" },
    ...
    concurrency: { executionMode: "fixed-concurrency", configuredMaxInflight: config.load.fixedConcurrency, effectiveMaxInflight: config.load.fixedConcurrency, targetConcurrency: config.load.fixedConcurrency, observedAtOperationStart: { ...distribution(concurrency), max: peakInflight } },
