...
export async function createOciNoSqlProvider({ config, table }) {
  ...
  return {
    async read(key) {
      ...
      if (!result.row || result.row.pk !== key.pk || result.row.sk !== key.sk || result.row.payload !== payload) throw Object.assign(new Error("Canonical item mismatch"), { name: "CorrectnessMismatch" });
      return { version: Number(result.row.version), attempts: 1, readUnits: result.consumedCapacity?.readUnits || 0, rateLimitDelayMs: result.consumedCapacity?.readRateLimitDelay || 0 };
    },
    async write(key, version) {
      const result = await client.put(table, { ...key, version, payload });
      return { version, attempts: 1, writeUnits: result.consumedCapacity?.writeUnits || 0, rateLimitDelayMs: result.consumedCapacity?.writeRateLimitDelay || 0 };
    },
