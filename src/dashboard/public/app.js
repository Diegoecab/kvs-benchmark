...
function stagePhaseLabel(schedule, index) {
  const current = Number(schedule?.[index]?.operationsPerSecond), previous = Number(schedule?.[index - 1]?.operationsPerSecond);
  const recovery = index === schedule.length - 1 && index > 0 && Number.isFinite(current) && Number.isFinite(previous) && current < previous;
  return `Stage ${index}${recovery ? " · recovery" : ""}`;
}
function sessionSchedule(session, status, startAt) {
  ...
  return `<div class="load-stage-grid">${session.loadSchedule.map((step, index) => { const start = offset, end = offset += Number(step.seconds); const phaseStatus = status === "complete" ? "complete" : status === "running" && elapsed >= end ? "complete" : status === "running" && elapsed >= start ? "running" : status === "not-run" ? "not-run" : "pending"; return `<article class="load-stage ${phaseStatus}"><span>${stagePhaseLabel(session.loadSchedule, index)}</span><b>${number(step.operationsPerSecond)} ops/s</b><small>${number(step.seconds)}s · T+${number(start)}s to T+${number(end)}s</small><i>${phaseStatus === "not-run" ? "Not run" : phaseStatus}</i></article>`; }).join("")}</div>`;
}
function drawStageBarChart(canvas, session, stageSummaries, { metric, offered = false, logarithmic = false, suffix = "" }) {
  ...
  values.forEach((stage, stageIndex) => {
    ...
    const step = session.loadSchedule[stageIndex]; context.fillStyle = "#b7c2d0"; context.textAlign = "center"; context.font = "11px ui-monospace, monospace"; context.fillText(stagePhaseLabel(session.loadSchedule, stageIndex), margin.left + groupWidth * (stageIndex + .5), height - 38); context.fillStyle = "#748197"; context.fillText(`${number(step.operationsPerSecond)} ops/s · ${number(step.seconds)}s`, margin.left + groupWidth * (stageIndex + .5), height - 20);
  });
}
...
function sessionResultTable(run, session, result, status) {
  ...
  const stageRows = (session.loadSchedule || []).flatMap((step, index) => Object.entries(result?.stageSummaries || {}).map(([target, stages]) => { const summary = stages[index] || {}, latency = summary.successfulServiceLatencyMs || {}; return `<tr><td>${stagePhaseLabel(session.loadSchedule, index)}</td><td>${number(step.operationsPerSecond)} ops/s · ${number(step.seconds)}s</td><td>${providerMark(target)} ${escapeHtml(runTargetLabel(target))}</td><td>${number(summary.accounted)} / ${number(summary.scheduled)}</td><td>${number(summary.failed)}</td><td>${number(summary.successfulOperationsPerSecond)} ops/s</td><td>${number(latency.p95)} ms</td><td>${number(latency.p99)} ms</td></tr>`; })).join("");
