...
const defaultImage = "ghcr.io/diegoecab/kvs-benchmark-runner@sha256:4fb423f7bbe2f90c36904855b3706c6c14fb4ce39b5bfe9731d88d5e5d16a32a";
...
function adbCredentialGuard(spec) {
  ...
  const requiredSeconds = Math.max(3_600, workloadSeconds + 7_200);
  ...
  return `sudo -n podman run --rm --user 0:0 -v /opt/kvs-dashboard:/secure:Z -e KVS_REQUIRED_SECONDS=${requiredSeconds} -e KVS_TABLE=${shellQuote(spec.adbTable)} --entrypoint node ${shellQuote(spec.image)} --input-type=module --eval '${javascript.replace("expiration_minutes:5256000", "expiration_minutes:360")}'`;
}
...
function cloudReportStageData(state) {
  return state.sessionResults.map(result => {
    const matrix = state.spec.matrix.find(item => item.id === result.id) || {};
    const schedule = (matrix.loadSchedule || []).map((step, index, values) => ({
      index,
      operationsPerSecond: Number(step.operationsPerSecond),
      seconds: Number(step.seconds),
      recovery: index === values.length - 1 && index > 0 && Number(step.operationsPerSecond) < Number(values[index - 1].operationsPerSecond),
    }));
    return { id: result.id, name: matrix.name || matrix.configName || result.id, repetition: result.repetition, readPercent: matrix.readPercent, writePercent: matrix.writePercent, schedule, targets: result.stageSummaries || {} };
  });
}
function safeInlineJson(value) { return JSON.stringify(value).replaceAll("<", "\\u003c").replaceAll(">", "\\u003e").replaceAll("&", "\\u0026"); }
function cloudStageReportSection(stageData) {
  if (!stageData.some(session => Object.keys(session.targets || {}).length)) return "";
  return `<section id="stage-performance"><h2>Results by offered-load stage</h2><p class="note">Each ramp is an independent measurement. A final load reduction is labeled <b>recovery</b> because it follows saturation and can retain backlog from the preceding stage.</p><div class="stage-controls"><label>Session <select id="stage-session"></select></label><label><input type="checkbox" data-stage-series="aws" checked><i class="swatch aws"></i>AWS DynamoDB</label><label><input type="checkbox" data-stage-series="adb" checked><i class="swatch adb"></i>ADB DynamoDB API</label><label><input type="checkbox" data-stage-series="ndcs" checked><i class="swatch ndcs"></i>OCI NoSQL</label><label><input type="checkbox" data-stage-series="offered" checked><i class="swatch offered"></i>Offered load</label></div><div class="stage-charts"><figure><figcaption>Successful throughput vs offered load · operations/s</figcaption><canvas id="stage-throughput" width="1040" height="320"></canvas></figure><figure><figcaption>Service success rate · percent</figcaption><canvas id="stage-success" width="1040" height="320"></canvas></figure><figure><figcaption>Successful P95 latency · milliseconds (log scale)</figcaption><canvas id="stage-latency" width="1040" height="320"></canvas></figure></div><div class="scroll"><table><thead><tr><th>Stage</th><th>Offered</th><th>Target</th><th>Accounted</th><th>Failed</th><th>Successful ops/s</th><th>P95 ms</th><th>P99 ms</th></tr></thead><tbody id="stage-results-body"></tbody></table></div><script type="application/json" id="stage-report-data">${safeInlineJson(stageData)}</script><script>
(function(){
  var sessions=JSON.parse(document.getElementById('stage-report-data').textContent),names={aws:'AWS DynamoDB',adb:'ADB DynamoDB API',ndcs:'OCI NoSQL'},colors={aws:'#ef8b22',adb:'#8b5cf6',ndcs:'#0f9ca5',offered:'#66727a'},select=document.getElementById('stage-session'),visible={aws:true,adb:true,ndcs:true,offered:true};
  function n(value,digits){return Number.isFinite(Number(value))?Number(value).toLocaleString('en-US',{maximumFractionDigits:digits==null?3:digits}):'—'}
  function esc(value){return String(value==null?'':value).replace(/[&<>"']/g,function(c){return {'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]})}
  sessions.forEach(function(session,index){var option=document.createElement('option');option.value=index;option.textContent=session.name+' · rep '+session.repetition;select.appendChild(option)});
  document.querySelectorAll('[data-stage-series]').forEach(function(box){box.addEventListener('change',function(){visible[box.dataset.stageSeries]=box.checked;render()})});select.addEventListener('change',render);
  function valueFor(summary,metric){if(metric==='throughput')return Number(summary.successfulOperationsPerSecond);if(metric==='success')return Number(summary.serviceSuccessRate)*100;return Number(summary.successfulServiceLatencyMs&&summary.successfulServiceLatencyMs.p95)}
  function draw(canvas,session,metric,includeOffered,logarithmic){var ctx=canvas.getContext('2d'),W=canvas.width,H=canvas.height,p={l:66,r:18,t:28,b:64},targets=['aws','adb','ndcs'].filter(function(t){return visible[t]&&session.targets[t]}),series=(includeOffered&&visible.offered?['offered']:[]).concat(targets),values=session.schedule.map(function(step,index){var row={};series.forEach(function(target){row[target]=target==='offered'?step.operationsPerSecond:valueFor((session.targets[target]||[])[index]||{},metric)});return row}),scale=function(v){return logarithmic?Math.log10(Math.max(0,v)+1):v},rawMax=Math.max.apply(Math,[1].concat(values.reduce(function(all,row){return all.concat(Object.keys(row).map(function(key){return row[key]}).filter(Number.isFinite))},[]))),max=scale(rawMax)*1.12;
    ctx.clearRect(0,0,W,H);ctx.fillStyle='#fff';ctx.fillRect(0,0,W,H);ctx.font='11px ui-monospace,monospace';ctx.textAlign='right';for(var tick=0;tick<=4;tick++){var fraction=(4-tick)/4,y=p.t+(H-p.t-p.b)*tick/4,raw=logarithmic?Math.pow(10,max*fraction)-1:max*fraction;ctx.strokeStyle='#dce4e8';ctx.beginPath();ctx.moveTo(p.l,y);ctx.lineTo(W-p.r,y);ctx.stroke();ctx.fillStyle='#63737d';ctx.fillText(n(raw,1),p.l-8,y+4)}if(!series.length)return;
    var plot=W-p.l-p.r,group=plot/values.length,gap=3,bar=Math.min(34,Math.max(5,(group-18)/series.length-gap));values.forEach(function(row,index){var total=series.length*(bar+gap)-gap,start=p.l+group*index+(group-total)/2;series.forEach(function(target,j){var raw=row[target],height=Number.isFinite(raw)?(H-p.t-p.b)*scale(raw)/max:0,x=start+j*(bar+gap),y=H-p.b-height;ctx.fillStyle=colors[target];if(target==='offered'){ctx.strokeStyle=colors[target];ctx.lineWidth=2;ctx.strokeRect(x,y,bar,height)}else ctx.fillRect(x,y,bar,height)});ctx.fillStyle='#304b5b';ctx.textAlign='center';ctx.fillText('S'+index+(session.schedule[index].recovery?' recovery':''),p.l+group*(index+.5),H-38);ctx.fillStyle='#63737d';ctx.fillText(n(session.schedule[index].operationsPerSecond,0)+' ops/s',p.l+group*(index+.5),H-20)});
  }
  function render(){var session=sessions[Number(select.value)||0];if(!session)return;draw(document.getElementById('stage-throughput'),session,'throughput',true,false);draw(document.getElementById('stage-success'),session,'success',false,false);draw(document.getElementById('stage-latency'),session,'latency',false,true);var rows=[];session.schedule.forEach(function(step,index){['aws','adb','ndcs'].forEach(function(target){if(!visible[target]||!session.targets[target])return;var summary=session.targets[target][index]||{},latency=summary.successfulServiceLatencyMs||{};rows.push('<tr><td>S'+index+(step.recovery?' · recovery':'')+'</td><td>'+n(step.operationsPerSecond,0)+' ops/s · '+n(step.seconds,0)+'s</td><td><i class="swatch '+target+'"></i>'+esc(names[target])+'</td><td>'+n(summary.accounted,0)+' / '+n(summary.scheduled,0)+'</td><td>'+n(summary.failed,0)+'</td><td>'+n(summary.successfulOperationsPerSecond,2)+'</td><td>'+n(latency.p95,3)+'</td><td>'+n(latency.p99,3)+'</td></tr>')})});document.getElementById('stage-results-body').innerHTML=rows.join('')}
  render();
})();
</script></section>`;
}
function legacyCloudReportHtml(state, preloadSection, rows, inventory, certificate) {
  const stageData = cloudReportStageData(state), workloadSeconds = state.spec.matrix.reduce((sum, item) => sum + Number(item.durationSeconds || 0), 0), operationsPerTarget = state.spec.matrix.reduce((sum, item) => sum + Number(item.scheduledOperationsPerTarget || 0), 0);
  const workloadDefinitions = [...new Map(state.spec.matrix.map(item => [item.name || item.configName, item])).values()].map(item => `<li><b>${item.name || item.configName}</b>: ${item.readPercent}% reads / ${item.writePercent}% writes, ${item.consistency} consistency, ${(item.loadSchedule || []).map((step, index, values) => `${step.operationsPerSecond}${index === values.length - 1 && index > 0 && Number(step.operationsPerSecond) < Number(values[index - 1].operationsPerSecond) ? " recovery" : ""}`).join(" → ")} ops/s, ${item.durationSeconds}s per repetition.</li>`).join("");
  return `<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>KVS cloud benchmark</title><style>:root{--ink:#172033;--muted:#63737d;--line:#dce4e8;--wash:#f4f7f8;--aws:#ef8b22;--adb:#8b5cf6;--ndcs:#0f9ca5;--offered:#66727a}*{box-sizing:border-box}body{max-width:1220px;margin:0 auto;padding:32px 22px;background:var(--wash);color:var(--ink);font:15px/1.5 system-ui,sans-serif}header,section{margin-bottom:18px;padding:20px;border:1px solid var(--line);border-radius:11px;background:#fff}h1,h2{margin-top:0}.ok{color:#087a55}.facts{display:grid;grid-template-columns:repeat(auto-fit,minmax(190px,1fr));gap:10px}.facts div{padding:12px;border:1px solid var(--line);border-radius:8px}.facts span{display:block;color:var(--muted);font-size:12px;text-transform:uppercase}.facts b{font-size:18px}table{width:100%;border-collapse:collapse}th,td{padding:9px;border-bottom:1px solid var(--line);text-align:left;white-space:nowrap}th{background:#edf3f6;font-size:12px;text-transform:uppercase}.scroll{overflow:auto;border:1px solid var(--line);border-radius:8px}pre{padding:14px;background:#f3f5f7;border-radius:8px;overflow:auto}.note{color:var(--muted)}.stage-controls{display:flex;flex-wrap:wrap;gap:12px;align-items:center;margin:14px 0}.stage-controls label{display:flex;gap:6px;align-items:center}.stage-controls select{max-width:390px;padding:7px}.swatch{display:inline-block;width:11px;height:11px;margin-right:5px;border-radius:3px;vertical-align:-1px}.swatch.aws{background:var(--aws)}.swatch.adb{background:var(--adb)}.swatch.ndcs{background:var(--ndcs)}.swatch.offered{height:0;border-top:2px dashed var(--offered);border-radius:0}.stage-charts{display:grid;grid-template-columns:1fr;gap:12px;margin:12px 0}.stage-charts figure{margin:0;padding:12px;border:1px solid var(--line);border-radius:8px}.stage-charts figcaption{margin-bottom:8px;font-weight:700}.stage-charts canvas{display:block;width:100%;height:auto}@media(max-width:700px){body{padding:12px}header,section{padding:14px}.stage-controls{align-items:flex-start;flex-direction:column}}</style></head><body><header><p>KVS BENCHMARK REPORT</p><h1>KVS cloud benchmark</h1><p class="ok"><b>COMPLETE</b> — ${state.sessionResults.length} synchronized matrix session(s) across ${state.spec.enabled.map(value => value.toUpperCase()).join(", ")} using ${state.spec.loadGeneratorCount || 1} load generator(s) per target.</p></header><section><h2>Benchmark definition</h2><div class="facts"><div><span>Sessions</span><b>${state.sessionResults.length}/${state.spec.matrix.length}</b></div><div><span>Measured traffic</span><b>${workloadSeconds / 60} min</b></div><div><span>Operations / target</span><b>${operationsPerTarget.toLocaleString("en-US")}</b></div><div><span>Item size</span><b>${state.spec.matrix[0]?.payloadBytes ?? "-"} B payload</b></div><div><span>Load generators</span><b>${state.spec.loadGeneratorCount || 1} per target</b></div></div><ul>${workloadDefinitions}</ul><p>Dataset SHA-256: <code>${certificate.observedSha256}</code>. Item size is reported as configured payload bytes and the provider-neutral largest logical JSON record; provider billing size can differ.</p></section>${cloudStageReportSection(stageData)}<section><h2>Benchmark resource inventory</h2><pre>${inventory}</pre></section><section>${preloadSection}<h2>Workload sessions</h2><div class="scroll"><table><thead><tr><th>Session</th><th>Target</th><th>Actual start UTC</th><th>Start skew ms</th><th>Completed</th><th>Item size</th><th>P95 ms</th><th>P99 ms</th><th>Max ms</th></tr></thead><tbody>${rows}</tbody></table></div></section><section><h2>Evidence</h2><p>The ZIP contains preload records, strong-read dataset certificates, operation-level NDJSON, telemetry, summaries, clock evidence, stage events, resource IDs, per-load-generator evidence, and SHA-256 manifest.</p></section></body></html>`;
}
const reportTargetNames = { aws: "AWS DynamoDB", adb: "ADB DynamoDB API", ndcs: "OCI NoSQL" };
function reportEscape(value) { return String(value ?? "").replace(/[&<>"']/g, character => ({ "&": "&amp;", "<": "&lt;", ">": "&gt;", '"': "&quot;", "'": "&#39;" })[character]); }
function reportNumber(value, digits = 3) { return Number.isFinite(Number(value)) ? Number(value).toLocaleString("en-US", { maximumFractionDigits: digits }) : "&mdash;"; }
function reportPercent(completed, scheduled) { return Number(scheduled) > 0 ? `${(100 * Number(completed) / Number(scheduled)).toFixed(3)}%` : "&mdash;"; }
function reportMedian(values) { const sorted = values.map(Number).filter(Number.isFinite).sort((left, right) => left - right); return sorted.length ? sorted[Math.floor(sorted.length / 2)] : null; }
function reportProviderMark(target) {
  if (target === "aws") return '<span class="provider-logo aws-logo" role="img" aria-label="AWS"><svg viewBox="0 0 48 28" aria-hidden="true"><text x="2" y="17">aws</text><path d="M4 22c10 6 26 5 38-2"/></svg></span>';
  return '<span class="provider-logo oracle-logo" role="img" aria-label="Oracle Cloud"><svg viewBox="0 0 40 28" aria-hidden="true"><rect x="3" y="6" width="34" height="16" rx="8"/></svg></span>';
}
function reportTargetTotals(state) {
  return Object.fromEntries((state.spec.enabled || []).map(target => {
    const summaries = state.sessionResults.map(session => session.summaries?.[target]).filter(Boolean);
    const scheduled = summaries.reduce((sum, value) => sum + Number(value.scheduled || 0), 0), failed = summaries.reduce((sum, value) => sum + Number(value.failed || 0), 0);
    return [target, { scheduled, failed, completed: scheduled - failed }];
  }));
}
function firstMaterialAdbDegradation(stageData) {
  const results = [];
  for (const name of [...new Set(stageData.map(session => session.name))]) {
    const sessions = stageData.filter(session => session.name === name && session.targets?.adb?.length), stages = sessions[0]?.schedule || [];
    for (let index = 0; index < stages.length; index += 1) {
      if (stages[index].recovery) continue;
      const samples = sessions.map(session => session.targets.adb[index]).filter(Boolean), scheduled = samples.reduce((sum, item) => sum + Number(item.scheduled || 0), 0), failed = samples.reduce((sum, item) => sum + Number(item.failed || 0), 0), medianP95 = reportMedian(samples.map(item => item.successfulServiceLatencyMs?.p95));
      if ((scheduled && failed / scheduled >= 0.0025) || Number(medianP95) >= 100) { results.push({ name, offered: stages[index].operationsPerSecond, failed, scheduled, medianP95 }); break; }
    }
  }
  return results;
}
function cloudExecutiveSummary(state, stageData) {
  const totals = reportTargetTotals(state), degradation = firstMaterialAdbDegradation(stageData), cards = Object.entries(totals).map(([target, value]) => `<article class="outcome ${value.failed ? "has-errors" : "clean"}">${reportProviderMark(target)}<div><span>${reportEscape(reportTargetNames[target] || target)}</span><strong>${reportPercent(value.completed, value.scheduled)}</strong><small>${reportNumber(value.failed, 0)} errors / ${reportNumber(value.scheduled, 0)} scheduled</small></div></article>`).join("");
  const boundaries = degradation.map(item => `<li><b>${reportEscape(item.name)}</b>: ${reportNumber(item.offered, 0)} offered ops/s; ${reportNumber(item.failed, 0)}/${reportNumber(item.scheduled, 0)} errors across repetitions; median successful P95 ${reportNumber(item.medianP95)} ms.</li>`).join("");
  const recoveries = stageData.flatMap(session => (session.schedule || []).map((stage, index) => ({ stage, summary: session.targets?.adb?.[index] })).filter(item => item.stage.recovery && item.summary));
  const recoveryScheduled = recoveries.reduce((sum, item) => sum + Number(item.summary.scheduled || 0), 0), recoveryFailed = recoveries.reduce((sum, item) => sum + Number(item.summary.failed || 0), 0), maxOffered = Math.max(0, ...stageData.flatMap(session => session.schedule.map(stage => Number(stage.operationsPerSecond || 0))));
  const strongRead = degradation.find(item => item.name.includes("strong-read"));
  return `<section class="executive"><div class="section-kicker">EXECUTIVE SUMMARY</div><h2>Decision summary</h2><p>This run completed and fully accounted ${reportNumber(state.spec.matrix.reduce((sum, item) => sum + Number(item.scheduledOperationsPerTarget || 0), 0), 0)} scheduled operations per target across ${state.sessionResults.length} synchronized sessions. It used ${state.spec.loadGeneratorCount || 1} equal load generators per target, ${state.spec.matrix[0]?.payloadBytes ?? "-"}-byte payloads, strong consistency, and ramps up to ${reportNumber(maxOffered, 0)} offered ops/s.</p><div class="outcomes">${cards}</div>${totals.adb && strongRead ? `<div class="finding"><b>Primary finding.</b> ADB DynamoDB API was comparable at the low-load baseline but did not sustain the intended matrix. Strong reads were clean at 100 and 200 ops/s, then showed service failures and a median successful P95 of ${reportNumber(strongRead.medianP95)} ms at 300 ops/s, despite three distinct load generators and verified egress identities.</div>` : ""}${boundaries ? `<h3>First material ADB degradation by workload</h3><p class="note">Material degradation is the first non-recovery stage with at least 0.25% errors across repetitions or a median successful P95 of at least 100 ms.</p><ul>${boundaries}</ul>` : ""}${recoveryScheduled ? `<p class="finding"><b>Recovery behavior.</b> After saturation, the 200 ops/s recovery stages still recorded ${reportNumber(recoveryFailed, 0)}/${reportNumber(recoveryScheduled, 0)} ADB errors, so those stages are not clean baseline measurements.</p>` : ""}<p><b>Conclusion:</b> the observed ADB service ceiling prevents a valid end-to-end comparison at the requested peak load. AWS DynamoDB and OCI NoSQL completed essentially the full matrix, while ADB completed ${totals.adb ? reportPercent(totals.adb.completed, totals.adb.scheduled) : "&mdash;"} successfully. The ADB limit must be raised or removed before rerunning the high-load comparison.</p></section>`;
}
function cloudInventorySection(inventory = {}) {
  const cards = Object.entries(inventory).map(([target, resource]) => {
    const resourceId = resource.tableArn || resource.autonomousDatabaseOcid || resource.tableOcid || "-";
    const configuration = target === "adb" ? [resource.databaseVersion, resource.workload, resource.computeCount && `${resource.computeCount} ${resource.computeModel || "compute"}`, resource.licenseModel === "BRING_YOUR_OWN_LICENSE" ? "BYOL" : resource.licenseModel].filter(Boolean).join(" / ") : target === "ndcs" ? "Provisioned OCI NoSQL table" : "Provisioned DynamoDB table";
    const runners = (resource.runnerInstances || []).map(runner => `<tr><td>${reportEscape(runner.source)}</td><td><b>${reportEscape(runner.displayName || "-")}</b><small class="resource-id">${reportEscape(runner.id)}</small></td><td>${reportEscape(runner.shape || "-")}<small>${reportNumber(runner.vcpus, 0)} vCPU / ${reportNumber(runner.memoryGB, 0)} GB</small></td><td>${reportEscape(runner.availabilityDomain || "-")}</td><td>${runner.privateIp ? `Private ${reportEscape(runner.privateIp)}` : "-"}${runner.egressIp ? `<small>Egress ${reportEscape(runner.egressIp)}${runner.egressIpVerified ? " (verified)" : ""}</small>` : runner.publicIp ? `<small>Public ${reportEscape(runner.publicIp)}</small>` : ""}</td></tr>`).join("");
    return `<article class="provider-card"><header>${reportProviderMark(target)}<div><h3>${reportEscape(reportTargetNames[target] || target)}</h3><span>${reportEscape(resource.region || "-")}</span></div></header><dl class="resource-facts"><div><dt>Table</dt><dd>${reportEscape(resource.tableName || "-")}</dd></div><div><dt>Configuration</dt><dd>${reportEscape(configuration)}</dd></div><div class="wide"><dt>Primary resource ID</dt><dd class="resource-id">${reportEscape(resourceId)}</dd></div></dl><h4>${reportNumber(resource.loadGeneratorCount || resource.runnerInstances?.length || 0, 0)} load generators</h4><div class="scroll"><table class="runner-table"><thead><tr><th>Source</th><th>Runner</th><th>Compute</th><th>Placement</th><th>Network identity</th></tr></thead><tbody>${runners}</tbody></table></div></article>`;
  }).join("");
  return `<section><div class="section-kicker">TEST ENVIRONMENT</div><h2>Benchmark resource inventory</h2><p class="note">Resource identifiers and network identities captured before execution. Lifecycle cleanup after the run does not change this immutable evidence.</p><div class="inventory-grid">${cards}</div></section>`;
}
function cloudReportHtml(state, preloadSection, rows, certificate) {
  const stageData = cloudReportStageData(state), workloadSeconds = state.spec.matrix.reduce((sum, item) => sum + Number(item.durationSeconds || 0), 0), operationsPerTarget = state.spec.matrix.reduce((sum, item) => sum + Number(item.scheduledOperationsPerTarget || 0), 0);
  const workloadDefinitions = [...new Map(state.spec.matrix.map(item => [item.name || item.configName, item])).values()].map(item => `<li><b>${reportEscape(item.name || item.configName)}</b>: ${item.readPercent}% reads / ${item.writePercent}% writes, ${reportEscape(item.consistency)} consistency, ${(item.loadSchedule || []).map((step, index, values) => `${step.operationsPerSecond}${index === values.length - 1 && index > 0 && Number(step.operationsPerSecond) < Number(values[index - 1].operationsPerSecond) ? " recovery" : ""}`).join(" &rarr; ")} ops/s, ${item.durationSeconds}s per repetition.</li>`).join("");
  return `<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>KVS cloud benchmark</title><style>:root{--ink:#172033;--muted:#63737d;--line:#dce4e8;--wash:#f4f7f8;--aws:#ef8b22;--adb:#8b5cf6;--ndcs:#0f9ca5;--offered:#66727a;--danger:#b42318;--success:#087a55}*{box-sizing:border-box}body{max-width:1220px;margin:0 auto;padding:32px 22px;background:var(--wash);color:var(--ink);font:15px/1.5 system-ui,sans-serif}header,section{margin-bottom:18px;padding:20px;border:1px solid var(--line);border-radius:11px;background:#fff}h1,h2{margin-top:0}.ok{color:var(--success)}.section-kicker{margin-bottom:5px;color:#57707d;font-size:11px;font-weight:800;letter-spacing:.14em}.facts,.outcomes{display:grid;grid-template-columns:repeat(auto-fit,minmax(190px,1fr));gap:10px}.facts div{padding:12px;border:1px solid var(--line);border-radius:8px}.facts span,.outcome span{display:block;color:var(--muted);font-size:12px;text-transform:uppercase}.facts b{font-size:18px}.outcome{display:flex;align-items:center;gap:12px;padding:14px;border:1px solid var(--line);border-radius:9px}.outcome strong{display:block;font-size:25px}.outcome small{display:block;color:var(--muted)}.outcome.has-errors strong{color:var(--danger)}.outcome.clean strong{color:var(--success)}.finding{margin:16px 0;padding:13px 15px;border-left:4px solid #4f78a8;background:#f3f7fb}.provider-logo{display:inline-flex;width:45px;height:30px;flex:0 0 auto}.provider-logo svg{width:100%;height:100%}.aws-logo text{font:bold 14px Arial;fill:#202832}.aws-logo path{fill:none;stroke:var(--aws);stroke-width:2.5;stroke-linecap:round}.oracle-logo rect{fill:none;stroke:#c74634;stroke-width:4}.inventory-grid{display:grid;gap:14px}.provider-card{padding:16px;border:1px solid var(--line);border-radius:10px}.provider-card>header{display:flex;align-items:center;gap:11px;margin:0 0 12px;padding:0;border:0}.provider-card h3,.provider-card h4{margin:0}.provider-card header span{color:var(--muted)}.resource-facts{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:9px;margin:0 0 14px}.resource-facts div{padding:10px;background:var(--wash);border-radius:7px}.resource-facts .wide{grid-column:1/-1}.resource-facts dt{color:var(--muted);font-size:11px;text-transform:uppercase}.resource-facts dd{margin:2px 0 0}.resource-id{display:block;overflow-wrap:anywhere;font:11px/1.45 ui-monospace,monospace;color:#52636e}.runner-table small{display:block;color:var(--muted)}table{width:100%;border-collapse:collapse}th,td{padding:9px;border-bottom:1px solid var(--line);text-align:left;white-space:nowrap}th{background:#edf3f6;font-size:12px;text-transform:uppercase}.scroll{overflow:auto;border:1px solid var(--line);border-radius:8px}.note{color:var(--muted)}.stage-controls{display:flex;flex-wrap:wrap;gap:12px;align-items:center;margin:14px 0}.stage-controls label{display:flex;gap:6px;align-items:center}.stage-controls select{max-width:390px;padding:7px}.swatch{display:inline-block;width:11px;height:11px;margin-right:5px;border-radius:3px;vertical-align:-1px}.swatch.aws{background:var(--aws)}.swatch.adb{background:var(--adb)}.swatch.ndcs{background:var(--ndcs)}.swatch.offered{height:0;border-top:2px dashed var(--offered);border-radius:0}.stage-charts{display:grid;grid-template-columns:1fr;gap:12px;margin:12px 0}.stage-charts figure{margin:0;padding:12px;border:1px solid var(--line);border-radius:8px}.stage-charts figcaption{margin-bottom:8px;font-weight:700}.stage-charts canvas{display:block;width:100%;height:auto}@media(max-width:700px){body{padding:12px}header,section{padding:14px}.stage-controls{align-items:flex-start;flex-direction:column}.resource-facts{grid-template-columns:1fr}.resource-facts .wide{grid-column:auto}}</style></head><body><header><p>KVS BENCHMARK REPORT</p><h1>KVS cloud benchmark</h1><p class="ok"><b>COMPLETE</b> &mdash; ${state.sessionResults.length} synchronized matrix session(s) across ${state.spec.enabled.map(value => value.toUpperCase()).join(", ")} using ${state.spec.loadGeneratorCount || 1} load generator(s) per target.</p></header>${cloudExecutiveSummary(state, stageData)}<section><div class="section-kicker">SCOPE</div><h2>Benchmark definition</h2><div class="facts"><div><span>Sessions</span><b>${state.sessionResults.length}/${state.spec.matrix.length}</b></div><div><span>Measured traffic</span><b>${workloadSeconds / 60} min</b></div><div><span>Operations / target</span><b>${operationsPerTarget.toLocaleString("en-US")}</b></div><div><span>Item size</span><b>${state.spec.matrix[0]?.payloadBytes ?? "-"} B payload</b></div><div><span>Load generators</span><b>${state.spec.loadGeneratorCount || 1} per target</b></div></div><ul>${workloadDefinitions}</ul><p>Dataset SHA-256: <code>${reportEscape(certificate.observedSha256)}</code>. Item size is reported as configured payload bytes and the provider-neutral largest logical JSON record; provider billing size can differ.</p></section>${cloudStageReportSection(stageData)}${cloudInventorySection(state.resourceInventory)}<section>${preloadSection}<h2>Workload sessions</h2><p class="note">Completed counts successful service operations; errors are shown separately. Accounted operations equal completed plus errors.</p><div class="scroll"><table><thead><tr><th>Session</th><th>Target</th><th>Actual start UTC</th><th>Start skew ms</th><th>Completed</th><th>Errors</th><th>Success rate</th><th>Item size</th><th>P95 ms</th><th>P99 ms</th><th>Max ms</th></tr></thead><tbody>${rows}</tbody></table></div></section><section><h2>Evidence</h2><p>The ZIP contains preload records, strong-read dataset certificates, operation-level NDJSON, telemetry, summaries, clock evidence, stage events, resource IDs, per-load-generator evidence, and SHA-256 manifest.</p></section></body></html>`;
}
export async function packageRun(state) {
  ...
  const rowsV2 = state.sessionResults.flatMap(session => { const matrix = state.spec.matrix.find(item => item.id === session.id) || {}; return Object.entries(session.summaries).map(([target, value]) => `<tr><td>${reportEscape(session.id)}</td><td>${reportEscape(reportTargetNames[target] || target.toUpperCase())}</td><td>${reportEscape(value.actualStartAt)}</td><td>${reportNumber(value.startSkewMs, 0)}</td><td>${reportNumber(value.completed, 0)} / ${reportNumber(value.scheduled, 0)}</td><td>${reportNumber(value.failed, 0)}</td><td>${reportPercent(value.completed, value.scheduled)}</td><td>${reportNumber(matrix.payloadBytes ?? value.dataset?.payloadBytes, 0)} B payload / ${reportNumber(matrix.logicalItemBytes ?? value.dataset?.logicalItemBytes, 0)} B logical max</td><td>${reportNumber(value.successfulServiceLatencyMs?.p95)}</td><td>${reportNumber(value.successfulServiceLatencyMs?.p99)}</td><td>${reportNumber(value.successfulServiceLatencyMs?.max)}</td></tr>`); }).join("");
  ...
  fs.writeFileSync(path.join(state.output, "index.html"), cloudReportHtml(state, preloadSection, rowsV2, certificate));
...
export class CloudAcceptanceRuns {
  ...
  refreshExternal() {
    for (const state of readRunStates(this.outputRoot)) {
      const current = this.runs.get(state.id), ownedAndActive = this.ownedRuns.has(state.id) && current && !terminalStatuses.has(current.status);
      if (ownedAndActive) continue;
      if (state.archiveFile && !fs.existsSync(state.archiveFile)) state.archiveFile = null;
      this.runs.set(state.id, state);
      if (terminalStatuses.has(state.status)) this.ownedRuns.delete(state.id);
    }
  }
