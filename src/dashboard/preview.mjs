...
export function previewMatrix(spec, { configDirectory }) {
  ...
  for (const file of files) {
    ...
    const loaded = applyRuntimeOverrides(readConfig(fullPath), overrides);
  ...
  return { schemaVersion: 1, generatedAt: new Date().toISOString(), infrastructure, targets, rows, totals: { tripletSessions: rows.length, targetExecutions: rows.reduce((sum, row) => sum + row.targets.length, 0), totalScheduledOperations, totalDatabaseMinutes } };
}
