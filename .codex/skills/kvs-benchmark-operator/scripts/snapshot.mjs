...
console.log(JSON.stringify({ runId: state.id, status: state.status, activeStage, targets: Object.keys(state.targetStatus || {}), currentSession: state.currentSession || null, sharedStartAt: state.sharedStartAt || null, completedSessions: state.sessionResults?.length || 0, totalSessions: state.spec?.matrix?.length || state.matrix?.length || 0, preload, live, lastEvent: state.logs?.at(-1) || null, error: state.error || null, downloadReady: Boolean(state.archiveFile) }, null, 2));
