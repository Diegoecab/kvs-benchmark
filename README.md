...
Then open `http://127.0.0.1:4177`. **Benchmark operations** is the home workspace: it shows the active run, execution history, pipeline, comparison charts, and packages. Select **New benchmark** only when a new run must be configured; this opens the five-step wizard. The dashboard and cloud controller run on Node.js on macOS, Linux, and Windows.

1. Keep **Use existing infrastructure** selected; the separate `kvs-benchmark-infra` adapter is not enabled yet.
2. Choose AWS and/or OCI, set one global **Load generators per target** count, then select exactly that many distinct regional runner VMs for every enabled product together with profiles, regions, compartments, tables, and provider-native evidence buckets.
...
The optional **Measure and compare preload performance** control schedules the canonical preload at one shared UTC T0. Configure the offered writes/s, maximum in-flight writes, transient-error attempt limit, and exponential-backoff base delay in step 4. Retries apply only to HTTP 429/5xx preload writes and are recorded in the preload evidence; they never change the workload profile's request-attempt policy. Evidence includes actual start and skew, elapsed time, requested/completed/failed writes, attempted and successful throughput, P50/P90/P95/P99/P99.9/max latency, attempts, retries, provider-reported write units, and operation-level NDJSON. A requested rate is an offered load, not an achieved-throughput result. Missing consumed-capacity data is reported as unavailable, and failed preload evidence is uploaded before the gate stops the pipeline.

Live values are provisional and intended for operational visibility. The active-run overview uses one provider-logo card per target to show the human-readable phase, region, table, load-generator fleet, and target state instead of making the raw state JSON the primary view. The status card identifies the active workload and its effective read/write mix, consistency, load model, execution mode, schedule or concurrency, duration, retry policy, timeout, dataset shape, configured payload size, provider-neutral logical item size, repetition, and shared T0. Per-target progress shows absolute accounting and percentage. The target-comparison charts plot the current observed throughput against offered load and rolling P95 latency; AWS DynamoDB, ADB DynamoDB API, OCI NoSQL, and offered-load series can be shown or hidden independently. Live throughput uses deltas inside the active workload window, excluding control-plane delivery time before T0. Persisted workload logs reconstruct the chart after a browser refresh. The run-timeline browser preserves every pipeline gate and every workload session as a selectable view: completed preload/certification results remain available while a later gate runs, and each workload schedule shows completed, active, and pending levels. Finalized open-loop sessions add conventional grouped bar charts inside **Execution performance** for successful throughput versus offered load, service success rate, and P95 latency at every load stage; the same target selector controls all comparison charts and exact values remain in the evidence table. The execution history keeps the active run pinned and exposes previous local and cloud runs as read-only records with status, configuration, per-target session results, elapsed time, and package download. Raw JSON remains available only inside the collapsed technical-details disclosure. Accepted comparisons use the complete operation evidence and final summaries in the downloadable ZIP.

Distributed runs keep the configured workload and preload rates as aggregate rates for each target. The deterministic global operation stream and canonical preload keyspace are partitioned by sequence across the selected VMs, every shard receives the same T0, and the controller reconstructs one target-level result. Per-source preload rate and maximum in-flight writes are derived from the aggregate controls, so increasing the generator count does not multiply the offered load. Raw files remain under `sources/source-NN/`, while the target root contains aggregate operations, telemetry, summary, effective configuration, clock evidence, offered-load stage summaries, and fleet metrics. Dataset certification uses the primary runner after the distributed preload; readiness validates every selected runner. Fixed-concurrency profiles distribute their global workers and reject a generator count greater than the configured worker count.
...
The same verified-resume path can reclaim a detached controller after all matrix sessions finish but before package completion. It revalidates the finalized matrix and regenerates the package without repeating cloud workloads.

Long OCI workloads use a fresh authenticated Object Storage client for the final evidence tree and retry the final upload if the live-progress uploader fails. Collection verifies the five mandatory workload artifacts; a missing OCI artifact is retried locally, republished once from the same runner directory, and recollected before the controller may classify the pipeline as failed. These recovery transitions are retained in `control/command-journal.ndjson`. Cloud ZIP creation hashes and stores files incrementally, so multi-session operation evidence is packaged without loading the full archive into dashboard memory. Each finalized open-loop session also stores `stage-summary.json` per target; selecting that historical session exposes accounting, errors, successful throughput, and p95/p99 latency for every offered-load stage.
...
The active workspace includes a dynamic **Benchmark definition** panel derived from the accepted run specification. It states the comparison objective, enabled targets, workload types and repetitions, measured traffic time, minimum matrix wall time including per-session T0 delivery windows, operations per target, canonical key/item size, distributed runner count, retry/timeout semantics, and the exact offered-load schedule for every workload family. Preparation, certification, evidence consolidation, validation, and packaging are explicitly excluded from the minimum matrix estimate instead of being hidden inside an imprecise ETA.

## Embedded benchmark operator

Each cloud session also captures a runner-infrastructure timeline. OCI runners use `oci_computeagent` CPU, memory, load, and network metrics; AWS uses native EC2 CPU, network, and status checks, with memory and OS load explicitly marked unavailable when the CloudWatch Agent is not installed. The workload telemetry additionally samples each runner's Linux CPU, memory, load, and network view for second-level live charts. Fleet charts use the maximum CPU/memory/load across selected generators and summed network throughput; every source report remains in the package. Infrastructure metric failure is diagnostic-only and cannot turn an otherwise valid workload into a failed benchmark.
...
- logical dataset, seed, payload, logical item size and key distribution;
...
- load-generator count, source identities and deterministic shard assignment;
