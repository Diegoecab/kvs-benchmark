...
Distributed runs keep the configured workload rate as the aggregate rate for each target. The deterministic global operation stream is partitioned by sequence across the selected VMs, every shard receives the same T0, and the controller reconstructs one target-level result. Raw files remain under `sources/source-NN/`, while the target root contains aggregate operations, telemetry, summary, effective configuration, clock evidence, offered-load stage summaries, and fleet metrics. Dataset preload and certification use the primary runner only; readiness validates every selected runner. Fixed-concurrency profiles distribute their global workers and reject a generator count greater than the configured worker count.
