...
test("HTML report is self-contained and exposes interactive provider labels", () => {
  ...
  assert.equal(data.sessions[0].targets.aws.throttling.affectedSeconds, 1); assert.equal(data.groups[0].durationSeconds, 3); assert.equal(data.groups[0].targets.aws.attemptedOperationsPerSecond, 1); assert.equal(data.sessions[0].targets.aws.errorCounts["HTTP 429 rate limit"], 1); assert.match(html, /Measured interval:/); assert.match(html, /avg ops\/min/); assert.match(html, /Fixture conclusion/); assert.match(html, /Supporting fixture/); assert.match(html, /data-series="aws"/); assert.match(html, /Offered load/); assert.match(html, /href="#methodology"/); assert.match(html, /Benchmark methodology/); assert.match(html, /Controlled scale-down and scale-up/); assert.match(html, /Consistency:/); assert.match(html, /Provisioned KVS capacity/); assert.match(html, /100 RCU \/ 100 WCU/); assert.match(html, /Exact execution windows \(UTC\)/); assert.match(html, /Scheduled start/); assert.match(html, /capacity\?\.events/); assert.doesNotMatch(html, /\[180,480\]/); assert.match(html, /Evidence index and reproducibility/); assert.match(html, /operations NDJSON/);
