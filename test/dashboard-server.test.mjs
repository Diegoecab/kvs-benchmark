...
test("dashboard serves bootstrap and protects preview with its launch token", async t => {
  const smoke = { id: "smoke-test", status: "complete", progress: { scheduled: 20, accounted: 20, completed: 20, failed: 0 }, summary: { harnessPassed: true } };
  const localSmokeRuns = { start: () => smoke, get: id => { assert.equal(id, smoke.id); return smoke; } };
  ...
  const startedSmoke = await fetch(`${origin}/api/local-smoke`, { method: "POST", headers: { "content-type": "application/json", "x-kvs-csrf": token }, body: "{}" });
