...
test("dashboard exposes a five-step wizard with contextual workload help", () => {
  ...
  assert.match(html, /value="non-failed">Hide failed/); assert.match(app, /run\.status !== "failed"/);
  assert.match(app, /function changeRunHistoryPage\(delta\)/); assert.match(app, /showHistoricalRun\(selected\.id\)/);
  assert.match(app, /async function syncServerActiveRun/); assert.match(app, /setInterval\(\(\) => void syncServerActiveRun/);
  assert.doesNotMatch(app, /Date\.now\(\) - runHistoryRefreshAt > 5000/);
  ...
  assert.match(app, /if \(runLocked && wizardActive\) showOperations\(\)/);
  ...
  assert.match(theme, /html \{[^}]*overflow-x: hidden/); assert.match(theme, /\.stage-tab-group \{[^}]*min-width: 0/);
