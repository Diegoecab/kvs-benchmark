...
test("OCI Run Command honors the wall-clock timeout without extra polling attempts", async t => {
  ...
  const calls = [];
  const executeCommand = async (_file, args) => { calls.push(args); return args.includes("create") ? "ocid1.instanceagentcommand.test\n" : JSON.stringify({ data: { "lifecycle-state": "ACCEPTED" } }); };
  ...
  assert.ok(calls.some(args => args.includes("cancel")));
});
