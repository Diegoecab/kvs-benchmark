...
**Live** uses the same immutable run configuration but refreshes more frequently. It displays aggregate per-target accounting, throughput, in-flight requests, latest latency, rolling P95, and a selectable timeline for AWS, ADB, OCI NoSQL, and offered load. Completed sessions add grouped per-ramp bars inside **Execution performance**. Target colors and provider marks share one selector across the live and per-stage charts. Live metrics are provisional until the evidence is finalized.
