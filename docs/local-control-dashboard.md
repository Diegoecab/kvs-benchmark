...
The active execution view begins with compact provider cards for AWS DynamoDB, ADB DynamoDB API, and OCI NoSQL. Each card shows its current human-readable phase, region, table, runner, and state. The **Run timeline** directly below the gate progress is independently selectable: it lists all pipeline gates plus every matrix session, preserves the operator's selection during polling, exposes completed preload and certification comparisons, and shows each workload's six offered-load levels. This lets an operator inspect an earlier stage while the current stage continues live. Raw controller JSON is intentionally secondary and remains in the collapsed **Technical run details** disclosure.
